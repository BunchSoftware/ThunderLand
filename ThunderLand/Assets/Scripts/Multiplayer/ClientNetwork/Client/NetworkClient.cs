    ...
    public class NetworkClient : ICommandClient, IDisposable
    {
        ...
        public bool IsConnected
        {
            get
            {
                try
                {
                    if (_TcpSocketClient != null && _TcpSocketClient.Connected)
                    {
                        if (_TcpSocketClient.Poll(0, SelectMode.SelectRead))
                        {
                            byte[] buff = new byte[1];
                            if (_TcpSocketClient.Receive(buff, SocketFlags.Peek) == 0)
                            {
        ...
        private void ReceiveCallback(IAsyncResult asyncResult)
        {
            if (TcpSocketClient.Connected)
            {
                ...
                Package package = null; /*= JsonConvert.DeserializeObject<Package>(data)*/
